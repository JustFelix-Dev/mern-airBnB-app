    ...
    return (
      <div className="absolute text-start inset-0 bg-black text-white h-full">
        <div className="bg-black p-8 grid justify-center items-center gap-4">
          <div>
            <h1 className="text-lg sm:text-xl md:text-3xl truncate max-w-xs sm:max-w-lg">
              Photos of {place.title}
            </h1>
            <button
              onClick={() => setShowAllPhotos(false)}
              className="fixed flex items-center right-12 top-8 flex gap-2 py-2 px-4 rounded-2xl bg-white text-black"
            >
              <AiFillCloseCircle />
              <span className="hidden md:block">Close Photo</span>
            </button>
          </div>
  ...
  return (
    <>
      <div className="mx-auto max-w-5xl relative">
        ...
        <button
          onClick={() => setShowAllPhotos(true)}
          className="absolute flex items-center bottom-2 right-2 bg-white font-medium text-black rounded-2xl border border-black py-2 px-4 "
        >
          <CgMenuGridO />
