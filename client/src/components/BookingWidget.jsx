...
const BookingWidget = ({ place }) => {
  ...
  let numOfNight = 0;
  if (checkIn && checkOut) {
    numOfNight = differenceInCalendarDays(
      new Date(checkOut),
      new Date(checkIn)
    );
  }
  ...
  const handleBooking = async () => {
    setIsLoading(true);
    if (!checkIn || !checkOut || !fullName || !mobile) {
    ...
    <>
      <div className="bg-white shadow-2xl border-t-2 border-primary p-4 rounded-2xl w-full">
        <p className="text-2xl text-center">
          ${place.price}/<span className="text-gray-500">night</span>
        </p>
        ...
        <button
          onClick={user ? handleBooking : handleRedirect}
          className="primary"
        >
          {!isLoading ? (
            <div>
              Reserve
              {numOfNight > 0 && <span> :${numOfNight * place.price}</span>}
            </div>
          ) : (
