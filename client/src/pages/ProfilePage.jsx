    ...
    <div className=' basicInfo  border-primary '>
      ...
      {user && (
        <div className='text-left p-4 leading-8 bg-white rounded-t-3xl font-semibold'>
          ...
          {user.admin && (
            <div className='bg-primary rounded-lg text-white text-sm py-1 max-w-[5rem] select-none px-4'>Admin</div>
          )}
          <div><span>Name: </span>{user?.name}</div>
          <div><span>Email: </span>{user?.email}</div>
          <div className='flex items-center gap-2 status'><span>Status : Active</span><div className='activeIcon'></div></div>
          <button onClick={logout} className='primary flex items-center gap-1 justify-center max-w-sm mt-2'>
            {isLoading ? (
              ...
              <div className='flex items-center gap-1'><span>Logout</span><span><svg width='20' height='20' fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
